...
import SignInWithCodeQR from "./signin-with-codeqr";
            ...
            >
              <button className="active:bg-gray-200 min-w-4 whitespace-nowrap flex items-center h-12 px-2 gap-2 sm:gap-3 font-medium text-sm rounded-lg hover:bg-gray-100 transition-colors">
                <Image
                  src={
                    user.project.logo ||
                    `${DICEBEAR_AVATAR_URL}${user.project.name}`
                  }
                  alt={`${user.project.name} avatar`}
                  width={24}
                  height={24}
                  className="rounded-full"
                />
                <span className="hidden sm:block">{user.project.name}</span>
